...
from utils import reverse_binarize
...
class randomHorizontalExpansion(object):
    ...
    def __call__(self, img):
        if isinstance(img, np.ndarray):
            ...
            shape = img.size
        img = self.horizontal_expansion(img, shape)
        self.scaling(img, shape)
        return img

    def _ratio(self):
        return np.random.rand() - 0.5  # {x∈R|x∈[-0.5, 0.5]}

    def horizontal_expansion(self, img, shape):
        img = img.resize((int(shape[0]*(1+self._ratio())), shape[1]), self.interpolation)
        return img

    def scaling(self, img, shape):
        ratio = self._ratio()
